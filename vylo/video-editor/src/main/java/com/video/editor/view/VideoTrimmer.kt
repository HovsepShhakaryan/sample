...
import com.video.editor.interfaces.*
...
class VideoTrimmer @JvmOverloads constructor(
    ...
    private fun setUpListeners() {
        ...
        icon_video_play.setOnTouchListener { _, event ->
    ...
    private fun onPlayerIndicatorSeekStart() {
        ...
        icon_video_play.setBackgroundResource(android.R.drawable.ic_media_play)
    ...
    private fun setProgressBarPosition(position: Float) {
        println((1000L * position / duration).toInt())
    ...
    fun save() {
        ...
        val inputPath = RealPathUtil.realPathFromUriApi19(context, fileUri)
        ...
        val outputFileUri = Uri.fromFile(
            File(
                root,
                "t_${Calendar.getInstance().timeInMillis}_" + file.nameWithoutExtension + if (mediaType == MediaType.VIDEO) ".mp4" else ".mp3"
            )
        )
        ...
        VideoCommands(context).trimMedia(
            mediaType,
    ...
    private fun togglePause() {
        if (video_loader.isPlaying) {
            ...
            icon_video_play.setBackgroundResource(android.R.drawable.ic_media_pause)
            if (resetBar) {
                resetBar = false
    ...
    private fun onVideoPrepared(mp: MediaPlayer) {
        ...
        if (videoProportion > screenProportion) {
            ...
            lp.height = (lp.width.toFloat() / videoProportion).toInt()
        } else {
            lp.width = (videoProportion * screenHeight.toFloat() * 0.8f).toInt()
            lp.height = (screenHeight * 0.8f).toInt()
        }
    ...
    private fun setTimeFrames(current: Float, end: Float) {
        val seconds = context.getString(R.string.short_seconds)
        textTimeSelection.text = String.format(
            "%s / %s",
            Utility.convertTimestampToString(current),
            Utility.convertTimestampToString(end)
        )
    }
    ...
    private fun onVideoCompleted() {
        video_loader.seekTo(startPos.toInt())
        icon_video_play.setBackgroundResource(android.R.drawable.ic_media_play)
        setProgressBarPosition(startPos)
    }
    ...
    fun setVideoURI(videoURI: Uri): VideoTrimmer {
        ...
        if (mediaType == MediaType.AUDIO) {
            audioWaveView.setSampleFrom(File(fileUri.path))
        }

        /*
        audioWaveView.onProgressChanged = object : SeekBarOnProgressChanged {
            override fun onProgressChanged(
                ...
                println(progress)
                audioWaveView.visibleProgress = progress / 10
            }
        }
         */
        return this
    ...
    fun setPreviewListner(listner: OnPreviewListner): VideoTrimmer {
        timeLineView.setPreviewListner(listner)
        return this
    }

    fun setMediaType(type: MediaType): VideoTrimmer {
        this.mediaType = type
        timeLineView.setMediaType(type)
        return this
    }
